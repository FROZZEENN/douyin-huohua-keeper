    ...
    if not outcomes and abort_reason:
        outcomes = [
            TargetOutcome(
                ...
                failure_kind=(
                    FailureKind.AUTH if auth_expired else FailureKind.TRANSIENT
                ),
    ...
    *,
    ...
    while True:
        ...
        if result.status is RunStatus.UNCERTAIN:
            LOGGER.warning(
                "结果不确定（消息可能已发出），不重试以免重复发送：%s", result.detail
            )
            return result.to_target_outcome(), False
...
def _craft_notice_text(
    *,
    ...
    if failed_now or uncertain_now:
        ...
        title = (
            f"⚠️ 有 {failed_now} 个人没发出去" if failed_now else "⚠️ 有人发送结果不确定"
        )
        body = (
            "\n".join(lines)
            + "\n\n"
...
def _build_plan(
    ...
    *,
    ...
    """
    targets = (
        contacts_override if contacts_override is not None else repo.enabled_contacts()
    )
