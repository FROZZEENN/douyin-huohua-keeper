...
def safe_id(value: str, *, kind: str = "ID") -> str:
    ...
    if not _SAFE_ID.fullmatch(text):
        raise ValueError(
            f"{kind} 不合法：{value!r} —— 只允许字母、数字、下划线、连字符，长度 1~64"
        )
    return text
...
class Repository:
    ...
    def _append_to_daily(self, report: RunReport) -> None:
        ...
        entry["last_uncertain"] = sum(
            1 for o in report.outcomes if o.status is RunStatus.UNCERTAIN
        )
