...
def fetch_qrcode(
    ...
    *,
    ...
    try:
        ...
        raise QrCodeUnavailableError(f"截取二维码失败：{type(exc).__name__}: {exc}") from exc
    ...
    **判定顺序很重要**，每一步的取舍见下面注释。
    ...
    if _has_session_cookie(page):
        ...
        return LoginPoll(
            ...
            detail=(
                "已拿到登录凭据，正在等待会话页加载…如果一直停在这里，点一下「刷新页面」；仍不行就重新扫码。"
            ),
        )
...
def verify_state_works(browser: Any, state_path: Any, *, timeout_ms: int = 45_000) -> tuple[bool, str]:
    ...
    context = None
    try:
        context = browser.new_context(**{**_context_kwargs(), "storage_state": str(state_path)})
    except Exception as exc:  # noqa: BLE001
    ...
    try:
        ...
        try:
            page.goto(LOGIN_URL, wait_until="domcontentloaded", timeout=max(timeout_ms, 15_000))
        except Exception as exc:  # noqa: BLE001
...
def _assert_capture_is_plausible(png_bytes: bytes, element: Any) -> None:
    ...
    if size is None:
        ...
        raise QrCodeUnavailableError("截取到的图片不是有效的 PNG，无法确认它是二维码。")
