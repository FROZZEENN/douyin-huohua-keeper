...
class Engine:
    ...
    def send_to_contact(
        ...
        **契约：任何一步失败都返回带 ``failure_kind`` 的 SendOutcome，不抛异常** ——
        ...
        """
        try:
            return self._send_to_contact(contact, message, dry_run=dry_run, allow_search=allow_search)
    ...
    def _send_to_contact(
        ...
        *,
        ...
        try:
            entry = session.call(
                lambda ctx: navigator.locate_conversation(ctx.page, contact.name, allow_search=allow_search)
            )
        ...
        try:
            ...
            baseline = session.call(
                lambda ctx: confirmer.snapshot_conversation(ctx.page, entry.name, expected_text=expected_hint)
            )
