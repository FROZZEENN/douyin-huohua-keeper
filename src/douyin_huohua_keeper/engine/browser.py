...
class BrowserSession:
    ...
    def call(self, fn: Callable[[BrowserContext], T], *, timeout: float | None = None) -> T:
        ...
        try:
            ...
            wait_seconds = timeout if timeout is not None else self.settings.browser_timeout_ms / 1000 + 30
            try:
    ...
    def _launch(self) -> BrowserContext:
        ...
        # 沙箱开不开、要不要绕开 /dev/shm —— 都看环境，不能无脑加（见 _needs_no_sandbox）
        in_container = _in_container()
        no_sandbox = _needs_no_sandbox(settings.chromium_no_sandbox)

        LOGGER.info(
            "启动浏览器：headless=%s slow_mo=%dms 沙箱=%s /dev/shm 绕行=%s",
            settings.headless,
            settings.slow_mo_ms,
            "关（容器或 root）" if no_sandbox else "开",
            "是" if in_container else "否",
        )
        ...
        launch_kwargs: dict[str, Any] = {
            ...
            "args": _chromium_args(no_sandbox=no_sandbox, small_shm=in_container),
        }
        ...
        _install_resource_blocker(page, settings.blocked_resource_types)
...
def _in_container() -> bool:
    ...
    为什么需要判断：容器里 Chromium 必须加 ``--no-sandbox``（没有 user namespace 权限）
    和 ``--disable-dev-shm-usage``（``/dev/shm`` 默认只有 64MB）。
    但在**桌面**上这两个都不需要 —— 尤其是 ``--no-sandbox`` 会白白关掉沙箱，
    等于让网页代码以你的用户权限运行。所以必须按环境区分，而不是无脑加上。
    """
    ...
    with contextlib.suppress(OSError):
        ...
        if any(mark in cgroup for mark in ("docker", "kubepods", "containerd", "podman")):
            return True
    # podman / 部分 CI 会设这两个环境变量

    return bool(os.environ.get("KUBERNETES_SERVICE_HOST") or os.environ.get("container"))  # noqa: SIM112


def _needs_no_sandbox(mode: str) -> bool:
    """按配置决定要不要关掉 Chromium 沙箱。

    ``auto``（默认）：容器里关；Linux 上以 root 跑也关（Chromium 会拒绝以 root 启动）；
    普通桌面**不关** —— 桌面没有任何理由牺牲沙箱。
    """
    if mode == "always":
        return True
    if mode == "never":
...
def _chromium_args(*, no_sandbox: bool, small_shm: bool) -> list[str]:
    """Chromium 启动参数。

    容器里跑 Chromium 有一串经典坑，前两个参数就是针对它们的 —— 但它们**只在
    需要时才加**（见 ``_needs_no_sandbox``）：桌面用户不该为容器的问题买单，
    更不该白白丢掉沙箱。

    - ``--no-sandbox``：容器内通常没有 user namespace 权限；桌面**不加**。
    - ``--disable-dev-shm-usage``：``/dev/shm`` 只有 64MB 时页面一大就崩；桌面不加。
    - ``--disable-gpu``：无头模式不需要 GPU，关掉能省掉一堆后端探测。
...
def _install_resource_blocker(page: Any, blocked: tuple[str, ...]) -> None:
    """拦掉用不上的资源类型（默认 ``font`` + ``media``）。

    实测结论（2026-09-19，/chat 流量画像）：那 27MB 里 **script 占 76%（21MB）**、
    stylesheet 8.9%、xhr 7.7%、fetch 6.9%，而 **image 只有 0.6%**、font/media 为 0。
    ⇒ **拦 font/media 在当前页面上省不到什么**（这是"页面改版后的保险"，不是省流量主力）。
    真要省流量得让 JS/CSS 走缓存 —— 每个 run 都新建 context，等于每次重下 21MB。

    用 ``page.route`` + ``abort``：请求**还没发出去**就被掐掉，连握手都省了。

    ⚠️ 默认**不拦 image**：二维码在页面上是 base64 的 data URI（不走网络），
    但万一哪天抖音把它换成网络图片，拦 image 会直接导致**登录失败**。
    为了省免费流量去冒"登录不了"的风险不划算 —— 想更激进自己往
    ``HUOHUA_BLOCK_RESOURCE_TYPES`` 里加 ``image`` 即可。
    """
    if not blocked:
        return
    wanted = {t.lower() for t in blocked}

    def _handler(route: Any, request: Any) -> None:
        try:
            # 只处理真正的网络请求；data: / blob: 一律放行（二维码就是 data URI）
            if str(request.url).startswith(("http://", "https://")) and (request.resource_type in wanted):
                route.abort()
            else:
                route.continue_()
        except Exception:  # noqa: BLE001
            # 拦截器绝不能把页面搞崩：任何意外都退化成"放行"
            with contextlib.suppress(Exception):
                route.continue_()

    try:
        page.route("**/*", _handler)
        LOGGER.info("已拦掉这些资源类型（省流量/提速）：%s", ", ".join(sorted(wanted)))
    except Exception as exc:  # noqa: BLE001
        # 拦截只是优化项 —— 挂掉也必须能正常跑
        LOGGER.warning("安装资源拦截失败（不影响功能，只是不省流量）：%s", exc)
...
def _explain_launch_failure(exc: Exception) -> str:
    ...
    if "Executable doesn't exist" in text or "playwright install" in text:
        return (
            "Chromium 未安装或路径不对。\n"
            "请在项目目录执行：playwright install chromium chromium-headless-shell\n"
            "⚠️ 两个都要装：无头模式用的是 chromium-headless-shell，只装 chromium 会一直报这个错。\n"
            "国内下载慢的话可先设 PLAYWRIGHT_DOWNLOAD_HOST，见 README「开始之前」。\n"
    ...
    if "Cannot allocate memory" in text or "Out of memory" in text:
        return "内存不足，Chromium 无法启动。\n建议至少 1GB 可用内存；容器里还要确认 /dev/shm 不小于 512MB。"
    if "Host system is missing dependencies" in text:
        return (
            "系统缺少 Chromium 所需的动态库。\n"
            "Debian/Ubuntu 上执行：sudo .venv/bin/playwright install-deps chromium\n"
            "然后重新装浏览器：playwright install chromium chromium-headless-shell"
        )
