...
def _compose_sticker(page: Any, sticker: str, *, dry_run: bool) -> ComposeResult:
    ...
    if emoji_button is None:
        raise ComposeError(f"找不到表情按钮。试过：{sel.describe_candidates(sel.EMOJI_BUTTON_CANDIDATES)}")
    ...
    if file_input is None:
        ...
        if button is not None:
            try:
                ...
                raise ComposeError(f"打开图片选择器失败：{type(exc).__name__}: {exc}") from exc

        raise ComposeError(
            f"找不到图片上传入口。试过：{sel.describe_candidates(sel.IMAGE_UPLOAD_CANDIDATES)}"
        )
