...
def _click_list_row(page: Any, entry: ConversationEntry, *, timeout_ms: int) -> None:
    ...
    try:
        ...
        raise ConversationOpenError(f"点击会话「{entry.name}」失败：{type(exc).__name__}: {exc}") from exc
        ...
        if result_item is not None:
            ...
            for index in range(count):
                ...
                if names_match(target, name):
                    return ConversationEntry(name=name, raw_text=text, index=index, source="search")
        time.sleep(0.2)
