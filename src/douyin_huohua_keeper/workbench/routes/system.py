...
async def system_config(request: Request) -> dict[str, Any]:
    ...
    return {
        ...
        "notify": {
            ...
            "configured": {
                name: _channel_configured(settings.notify, name)
                for name in settings.notify.KNOWN_CHANNELS
            },
        },
        "scheduler": {
...
def run_check(request: Request) -> dict[str, Any]:
    ...
    if settings.workbench.token:
        if len(settings.workbench.token) >= 12:
            ...
            checks.append(
                _warn("访问令牌", f"已设置但偏短（{len(settings.workbench.token)} 字符）", "建议至少 24 位随机字符。")
            )
    else:
        checks.append(
            _fail("访问令牌", "未设置 —— 工作台无鉴权开放", "设置 HUOHUA_TOKEN，否则任何能访问端口的人都能操作。")
        )
    ...
    if account_state["present"] and not account_state["expired"]:
        ...
        checks.append(
            _fail("登录态文件", account_state["detail"], "去「账号」页重新扫码登录。")
        )
    else:
        checks.append(
            _warn("登录态文件", account_state["detail"], "这是首次部署的正常状态 —— 去「账号」页扫码绑定即可。")
        )
    ...
    if enabled:
        ...
        checks.append(
            _warn("收件人", "没有启用的收件人", "去「收件人」页添加，或从会话列表同步。")
        )
    ...
    if messages:
        ...
        checks.append(
            _warn("消息池", "为空，发送时会用默认文案「早」", "去「任务」页配置几条常用文案。")
        )
    ...
    try:
        ...
        if stale:
            checks.append(
                _warn("临时文件", f"发现 {len(stale)} 个残留 .tmp", "通常是历史崩溃留下的；重启服务会自动清理。")
            )
        else:
