    ...
    if payload.merge:
        ...
        for name in names:
            if name not in existing:
                ...
                # 要发给谁，在列表里单独打开即可。
                merged.append(Contact(name=name, enabled=False, is_group=groups.get(name, False)))
                added.append(name)
        if added:
...
def list_friends(request: Request, limit: int = 300) -> dict[str, Any]:
    ...
    try:
        ...
        details = engine.session.call(lambda ctx: navigator.read_conversation_details(ctx.page, limit=limit))
...
async def reset_today(request: Request, payload: ResetTodayRequest) -> dict[str, Any]:
    ...
    **这是一个危险操作**：清掉之后定时任务会认为今天还没发过，
    ...
    if not payload.confirm:
        raise HTTPException(
            status_code=422,
            detail=("这个操作会让今天的防重复失效，可能重复发送。确认要这么做请传 confirm=true。"),
        )
