...
async def clear_account_state(request: Request, account_id: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "account_id": target,
        "detail": (
            f"已删除 {target} 的登录态文件" if removed else f"账号 {target} 本来就没有登录态"
        ),
    ...
    try:
        ...
        return {
            ...
            "hint": (
                ...
                "（执行 playwright install chromium）、或抖音改了登录页结构。"
            ),
        }
...
class QrActRequest(BaseModel):
    ...
    index: int | None = Field(
        ...
        le=20,
        description=(
            "要操作的输入框序号（与 /qr/page 返回的 inputs 顺序一致）。"
            "留空时自动选「最像验证码」的那个框。"
        ),
    )
...
def qr_act(request: Request, payload: QrActRequest) -> dict[str, Any]:
    ...
    **可见反馈**：填完验证码能立刻看到「字确实进去了」，而不是「点了没反应」。
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"操作登录页失败：{type(exc).__name__}: {exc}"
        ) from exc
...
def _capture_page(ctx: Any) -> dict[str, Any]:
    ...
    JPEG(q=60) 只有几十 KB —— 手机上每 5 秒刷新一次的体感差别很大
    （PNG 会明显卡顿、也更容易把浏览器线程占住）。
    """
    page = ctx.page
    shot = page.screenshot(type="jpeg", quality=60, timeout=15_000)
...
def _apply_action(page: Any, payload: QrActRequest, *, last_click: Any) -> dict[str, Any]:
    ...
    if action == "click":
        ...
        return {
            "detail": (
                "已点击，光标已在输入框里 —— 现在可以输入了"
                if editable
                else f"已点击 ({x:.0f}, {y:.0f})"
            ),
            "focused": info,
