...
class TestLevelFromState:
    ...
    def test_custom_thresholds(self) -> None:
        ...
        assert level_from_state(consecutive_failures=5, warn_threshold=5, critical_threshold=10) is AlertLevel.WARNING
        assert level_from_state(consecutive_failures=9, warn_threshold=5, critical_threshold=10) is AlertLevel.WARNING
        assert level_from_state(consecutive_failures=10, warn_threshold=5, critical_threshold=10) is AlertLevel.CRITICAL
    ...
    def test_no_failures_stays_normal(self) -> None:
        assert (
            level_from_state(consecutive_failures=0, failed_now=0, uncertain_now=0)
            is AlertLevel.NORMAL
        )
...
class TestUrlEscaping:
    ...
    def test_bark_escapes_special_characters(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        url = self._capture_bark_url(
            monkeypatch, AlertLevel.NORMAL, "标题 空格&符号", "正文/斜杠?问号"
        )
