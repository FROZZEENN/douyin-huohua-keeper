...
class TestScheduleSourceOfTruth:
    def test_schedule_from_config_reads_tasks_json(self, repo: Repository, tmp_path: Any) -> None:
        repo.save_schedule(Schedule(enabled=True, hour=21, minute=5, jitter_minutes=3))
...
class FakeEngine:
    ...
    def send_to_contact(
        self, contact: Any, message: Any, *, dry_run: bool, allow_search: bool
    ) -> FakeSendOutcome:
        self.calls.append((contact.name, message.content))
        outcome = (
            self.script.pop(0)
            if self.script
            else FakeSendOutcome(contact_name=contact.name, status=RunStatus.SUCCESS, detail="ok")
        )
        return outcome
...
class TestRunOnce:
    ...
    def _patch_engine(self, monkeypatch: Any, tmp_path: Any, repo: Repository) -> None:
        ...
        monkeypatch.setattr(
            ...
            lambda path: types.SimpleNamespace(expired=False, needs_renewal=False, detail="ok"),
        )
...
class TestMissedDayWarning:
    ...
    def test_warns_when_today_was_missed(self, tmp_path: Any, monkeypatch: pytest.MonkeyPatch) -> None:
        sent = self._patch(monkeypatch)
        # 固定注入「当天 00:20」—— 已过 00:00 + 15 分钟宽限期，且与真实时钟无关
        schedule = Schedule(enabled=True, hour=0, minute=0, jitter_minutes=0)
        repo = self._Repo(tmp_path, succeeded=False)

        jobs_mod._warn_if_today_missed(schedule, repo=repo, now=datetime(2026, 9, 19, 0, 20))
    ...
    def test_silent_before_todays_time(self, tmp_path: Any, monkeypatch: pytest.MonkeyPatch) -> None:
        """今天还没到点（或刚到点 15 分钟内）时不该报警。"""
        sent = self._patch(monkeypatch)
        schedule = Schedule(enabled=True, hour=23, minute=59, jitter_minutes=0)
        repo = self._Repo(tmp_path, succeeded=False)

        jobs_mod._warn_if_today_missed(schedule, repo=repo, now=datetime(2026, 9, 19, 23, 59, 30))

        assert sent == []

    def test_silent_when_schedule_disabled(self, tmp_path: Any, monkeypatch: pytest.MonkeyPatch) -> None:
        sent = self._patch(monkeypatch)
