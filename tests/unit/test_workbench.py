...
from douyin_huohua_keeper.workbench import app as app_mod
...
def _settings(tmp_path: Any, *, token: str = TOKEN, allowed_ips: tuple[str, ...] = ()) -> Any:
    base = load_settings()
    workbench = dataclasses.replace(base.workbench, token=token, allowed_ips=allowed_ips)
...
class TestAuthMiddleware:
    ...
    def test_token_via_header(self, client: TestClient) -> None:
        assert client.get("/api/contacts", headers={"X-Huohua-Token": TOKEN}).status_code == 200

    def test_token_via_bearer(self, client: TestClient) -> None:
        assert client.get("/api/contacts", headers={"Authorization": f"Bearer {TOKEN}"}).status_code == 200
...
class TestAuthBackoff:
    ...
    def _reset(self, monkeypatch: pytest.MonkeyPatch) -> None:
        # 计数器是模块级状态，测试之间必须隔离
        app_mod._AUTH_FAILS.clear()
        monkeypatch.setattr(app_mod, "AUTH_FAIL_LIMIT", 3)
        monkeypatch.setattr(app_mod, "AUTH_LOCKOUT_SEC", 60.0)

    def test_repeated_failures_are_throttled(self, client: TestClient) -> None:
        codes = [client.get("/api/system/overview").status_code for _ in range(4)]
        assert codes[:3] == [401, 401, 401], codes
        assert codes[3] == 429, codes

    def test_success_resets_the_counter(self, client: TestClient) -> None:
        """偶发手误不该累积成误伤 —— 认证成功就清零。"""
        assert client.get("/api/system/overview").status_code == 401
        assert client.get("/api/system/overview").status_code == 401

        assert client.get("/api/system/overview", headers={"X-Huohua-Token": TOKEN}).status_code == 200

        # 没清零的话，这里会是 [401, 429]
        codes = [client.get("/api/system/overview").status_code for _ in range(2)]
        assert codes == [401, 401], codes
...
class TestIpAllowlist:
    ...
    def test_allowlist_applies_even_with_valid_token(self, tmp_path: Any) -> None:
        ...
        app = create_app(_settings(tmp_path, allowed_ips=("10.0.0.0/8",)))
        client = TestClient(app)
        assert client.get("/api/contacts", headers={"X-Huohua-Token": TOKEN}).status_code == 403
...
class TestContactsCrud:
    ...
    def test_add_and_list(self, client: TestClient) -> None:
        ...
        data = client.get("/api/contacts", headers={"X-Huohua-Token": TOKEN}).json()
        assert data["count"] == 1
    ...
    def test_add_same_name_updates_not_duplicates(self, client: TestClient) -> None:
        ...
        resp = client.post("/api/contacts", json={"name": "阿明", "note": "改备注"}, headers=headers)
    ...
    def test_add_empty_name_is_422(self, client: TestClient) -> None:
        resp = client.post("/api/contacts", json={"name": "  "}, headers={"X-Huohua-Token": TOKEN})
    ...
    def test_patch_renames(self, client: TestClient) -> None:
        ...
        resp = client.patch("/api/contacts/旧名", json={"name": "新名"}, headers=headers)
        assert resp.status_code == 200
    ...
    def test_patch_missing_is_404(self, client: TestClient) -> None:
        resp = client.patch("/api/contacts/不存在", json={"name": "x"}, headers={"X-Huohua-Token": TOKEN})
    ...
    def test_delete(self, client: TestClient) -> None:
        ...
        assert client.delete("/api/contacts/要删的", headers=headers).status_code == 200
        assert client.delete("/api/contacts/要删的", headers=headers).status_code == 404
...
class TestReadOnlyEndpoints:
    def test_tasks_reflect_tasks_json(self, client: TestClient, tmp_path: Any) -> None:
        ...
        data = client.get("/api/tasks", headers={"X-Huohua-Token": TOKEN}).json()
    ...
    def test_schedule_endpoint(self, client: TestClient, tmp_path: Any) -> None:
        ...
        data = client.get("/api/tasks/schedule", headers={"X-Huohua-Token": TOKEN}).json()
...
class TestAuthBackoffSize:
    """失败计数的规模必须可控 —— 客户端 IP 不受我们控制。"""

    def test_tracked_clients_are_capped(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """扫描器换着 IP 来一次就走：那些记录不能被永久留着。"""
        app_mod._AUTH_FAILS.clear()
        monkeypatch.setattr(app_mod, "AUTH_FAIL_MAX_TRACKED_CLIENTS", 5)
        for i in range(50):
            app_mod._record_auth_failure(f"10.0.0.{i}")
        assert len(app_mod._AUTH_FAILS) <= 5

    def test_stale_entries_are_dropped_first(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """超限时**先清过期的**，新鲜的保住 —— 不能一刀切把正在挡的那几个也丢了。"""
        app_mod._AUTH_FAILS.clear()
        monkeypatch.setattr(app_mod, "AUTH_FAIL_MAX_TRACKED_CLIENTS", 3)

        now = time.monotonic()
        app_mod._AUTH_FAILS["old-1"] = [now - 9999]
        app_mod._AUTH_FAILS["old-2"] = [now - 9999]
        app_mod._AUTH_FAILS["fresh-1"] = [now]
        app_mod._AUTH_FAILS["fresh-2"] = [now]

        app_mod._record_auth_failure("fresh-3")  # 现在 5 条 > 上限 3，触发清理

        assert "old-1" not in app_mod._AUTH_FAILS
        assert "old-2" not in app_mod._AUTH_FAILS
        assert "fresh-1" in app_mod._AUTH_FAILS
        assert "fresh-2" in app_mod._AUTH_FAILS

    def test_limit_zero_disables_throttling(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """AUTH_FAIL_LIMIT=0 时不能抛 IndexError —— 那会变成 500 而不是 401。"""
        app_mod._AUTH_FAILS.clear()
        monkeypatch.setattr(app_mod, "AUTH_FAIL_LIMIT", 0)
        assert app_mod._auth_locked("10.0.2.1") is False
