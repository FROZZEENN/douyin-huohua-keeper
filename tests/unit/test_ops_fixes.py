...
class FakeNavEngine:
    ...
    def __init__(self, page: Page, *, logged_in: bool = True) -> None:
        ...
        self.session = types.SimpleNamespace(call=lambda fn: fn(types.SimpleNamespace(page=page)))
...
class TestNotifyTestEndpoint:
    ...
    def test_successful_channel_reports_ok(self, tmp_path: Any, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestSelfCheck:
    ...
    def test_check_fails_when_notify_broken(self, tmp_path: Any, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestContactsPagination:
    ...
    def test_write_endpoint_returns_only_first_page(self, tmp_path: Any) -> None:
        ...
        resp = client.post("/api/contacts", json={"name": "新来的", "enabled": True}, headers=HDR).json()
...
class TestEngineIdleRecycle:
    ...
    def test_reaps_after_idle_timeout(self, tmp_path: Any) -> None:
        ...
        calls = {"stopped": 0}
        state._engine = types.SimpleNamespace(stop=lambda: calls.__setitem__("stopped", calls["stopped"] + 1))
...
class TestCookieRenewalJudgement:
    ...
    def _state_file(tmp_path: Path, *, age_days: float) -> Path:
        ...
        path.write_text(json.dumps({"cookies": [{"name": "sessionid", "value": "x"}]}), encoding="utf-8")
